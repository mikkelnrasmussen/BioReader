...
retrive_articles <- function(pmidPositive, pmidNegative, pmidTBD,
  ...
  if (is.character(pmidPositive) && pmidPositive == "") {
    pmidPositive <- NULL
  }
  if (is.character(pmidNegative) && pmidNegative == "") {
    pmidNegative <- NULL
  }
  if (is.character(pmidTBD) && pmidTBD == "") {
    pmidTBD <- NULL
  }
  ...
  if (shiny_input) {
    ...
    pmidPositive <- if (!is.null(pmidPositive)) {
      str_extract_all(pmidPositive, "\\d+")
    }
    pmidNegative <- if (!is.null(pmidNegative)) {
      str_extract_all(pmidNegative, "\\d+")
    }
    pmidTBD <- if (!is.null(pmidTBD)) {
      str_extract_all(pmidTBD, "\\d+")
    }
  }
  ...
  if (progress) {
    setProgress(
      message = "Downloading articles from PubMed...",
  ...
  for (i in seq(1, N, by = chnk_len)) {
    ...
    maxTimes <- 5
    req <- RETRY(
      verb = "GET", url = url, times = maxTimes,
  ...
  if (verbose) {
    ...
    print(paste0(
      "Procent of PubMed articles with abstract: ",
...
pubmed_articles <- function(pmidPositive, pmidNegative, pmidTBD, verbose = FALSE,
  ...
  retrieve_pubmed <- function(x) {
    ...
    my_query_pl <- lapply(1:numCores, function(i) {
      paste(split_query[[i]],
        collapse = " "
      )
    })
  ...
  if (verbose) {
    ...
    print(paste0(
      "Procent of PubMed articles retrieved: ",
      round(frac_retrieved, 4) * 100, "%"
    ))
  }
...
train_classifiers <- function(train_data, eval_metric, verbose = FALSE,
                              ...
                              model_names = c(
                                "bag_mars", "bag_tree", "bart", "xgboost",
                                "c5", "dt", "fdm", "ldm", "rdm",
                                "logit", "mars", "nnet", "mr",
  ...
  model_specs <- list(
    bag_mars = bag_mars_spec, bag_tree = bag_tree_spec,
...
classifier_predict <- function(final_model_fit, test_data) {
  ...
  pred <- bind_cols(
    ...
    predict(final_model_fit,
      new_data = test_data[, c("pmid", "abstract")],
      type = "prob"
    )
  )
...
classify_articles <- function(data_separated, metric = "roc_auc", fold = 5,
  ...
  prediction_results <- classifier_predict(
    final_model_fit = best_model,
...
evaluate_models <- function(pred_train = NULL, test_data = NULL, fitted_models = NULL,
  ...
  models_map <- data.frame(
    ...
    Model = c(
      "Bagged MARS", "Bagged Decision Tree",
  ...
  if (!is.null(pred_train)) {
    ...
    train_plot <- df.roc.train %>%
      ...
      geom_text(
        data = auc_metrics, x = 0.75, y = 0.25, size = 3,
        aes(label = label.auc), inherit.aes = F
      ) +
      labs(
        title = "ROC curves across the 10-fold cross-validation for training data",
  ...
  if (!is.null(fitted_models)) {
    ...
    for (model in models_map$model_name) {
      current_results <- classifier_predict(
        final_model_fit = fitted_models[[model]],
        test_data = testing_data
      )$pred
    ...
    test_plot <- df.roc.test %>%
      ...
      theme(legend.position = "none") +
      geom_text(
        data = all_metrics, x = 0.75, y = 0.25, size = 3,
        aes(label = label.auc), inherit.aes = F
      ) +
      labs(
    ...
    df.lines <- data.frame(
      X = c(rep(df.best.spec$max_spec, 2)),
      Y = c(
        rep(0, length(df.best.spec$Model)),
    ...
    train_plot_sens_thres <- df.roc.test %>%
      ...
      geom_line(
        data = df.lines, aes(x = 1 - X, y = Y, group = Model), colour = "red",
        alpha = 0.5
      ) +
      geom_text(
        data = df.best.spec, x = 0.65, y = 0.15, size = 3,
    ...
    DT::datatable(df.summarized,
      colnames = c(
        "Model", "Specificity at 95% sensitivity",
        "AUC score"
      ),
      rownames = FALSE,
      options = list(
    ...
    train_plot_compare <- df.roc.test.compare %>%
      ...
      geom_abline(intercept = 0, slope = 1, lty = 3) +
      labs(
        title = "Comparison of ROC curves for the 3 best classifiers and the 3 worst classifiers",
